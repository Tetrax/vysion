...
def verifier_vs_extintf_any(config_lines):
    """
    Vérifie que les Virtual Servers n'ont pas 'any' pour 'set extintf'
    Retourne : (result, conform)
    """
    in_bloc = False
    niveau_imbrication = 0
    ...
    for line in config_lines:
        ...
        if niveau_imbrication == 1 and line_strip == "next":
            if edit_name and is_vs and extintf_any:
                vs_extintf_any.append(edit_name)
            edit_name = None
            is_vs = False
            extintf_any = False
            continue
    ...
    if not vs_extintf_any:
        result = "Aucun Virtual Server a 'ANY' comme interface externe"
        conform = True
    else:
        result = f"Virtual Server(s) avec 'ANY' sur l'interface externe : {', '.join(vs_extintf_any)}"
        conform = False
    ...
    for line in config_lines:
        ...
        if line == 'config log fortianalyzer setting':
            ...
            if line.startswith('set status'):
                ...
                # Extraction du serveur (compatibilité avec ou sans guillemets)
                if '"' in line:
                    parts = line.split('"')
                    if len(parts) > 1:
                        faz_server = parts[1]
        ...
        # Bloc FortiAnalyzer Cloud
            ...
            if line.startswith('set status'):
                parts = line.split()
                if len(parts) >= 3:
                    faz_cloud_status = parts.lower()

    # Résultats
    if faz_status == "enable" and faz_server:
        return f"Le FortiGate est synchronisé avec le FortiAnalyzer {faz_server}. Etat de la synchronisation à vérifier.", True
    elif faz_cloud_status == "enable":
        return "Le FortiGate est synchronisé avec FortiAnalyzer Cloud. Etat de la synchronisation à vérifier", True
    else:
        return "Le FortiGate n'est pas synchronisé avec un FortiAnalyzer ni avec FortiAnalyzer Cloud", False
...
# Fonction qui vérifie si la version du firmware est vulnérable (CVE connues)
def est_version_concernee_par_cve(version):
    base_url = "https://www.fortiguard.com/psirt?filter=1&product=FortiOS-6K7K%2CFortiOS&version="
    url = f"{base_url}{version}&severity=5&severity=4"
    ...
    # Recherche d'éléments avec la classe "cve"
    cve_elements = soup.find_all("b", class_="cve")

    # Extraction des CVE trouvées
    ...
    if len(cve_list) > 3:
        ...
        message = f"Version {version} concernée par des CVE 'high' et/ou 'critiques' connues : {cve_details}. \n Source : https://www.fortiguard.com/psirt"
    else:
        message = f"Version {version} non concernée par des CVE 'high' et/ou 'critiques' connues\n Source : https://www.fortiguard.com/psirt"

    return message, len(cve_list) > 0



def verifier_modele_fortigate_eol(modele):
    """
    Vérifie si un modèle FortiGate est en fin de vie (EOL) en scrappant la page Fortinet.
    Retourne: (result, conform)
      - result: message d'information
      - conform: True si conforme (non EOL), False si non conforme (EOL), None si erreur/non trouvé
    """

    def normalize_model_for_eol(model_from_config):
        """Convertit le modèle de config vers le format attendu sur la page EOL (identité ici)."""
        if not model_from_config:
            return ""
        model_from_config = str(model_from_config).strip().upper()
        # Si tu veux des règles supplémentaires de normalisation, ajoute-les ici
        return model_from_config

    if not modele:
        result = "Modèle non fourni ou non trouvé"
        conform = None
        print(f"Result: {result}")
        print(f"Conform: {conform}")
        return result, conform

    # Considérer toute série 'G' comme supportée (règle métier spécifique)
    if str(modele).strip().upper().endswith('G'):
        result = f"Modèle {modele} (série G) : conforme, dernière gamme Fortinet, supporté."
        conform = True
        print(f"Result: {result}")
        print(f"Conform: {conform}")
        return result, conform

    normalized_model = normalize_model_for_eol(modele)
    ...
    try:
        ...
        result = f"Modèle {modele} : Impossible de vérifier le statut end of life (erreur réseau)"
        conform = None
        print(f"Result: {result}")
        print(f"Conform: {conform}")
        return result, conform
    ...
    # Fonction de nettoyage des cellules (retire les espaces insécables)
    def clean(td):
        return td.get_text(strip=True).replace("\xa0", " ")

    # Préparer un motif robuste : accepte "FortiGate-40F" ou "40F" comme mot entier
    # Ex: pour 40F -> r'\b(?:FortiGate-)?40F\b'
    pat = re.compile(r'\b(?:FortiGate-)?' + re.escape(normalized_model) + r'\b', flags=re.IGNORECASE)

    for row in table_rows:
        cells = row.find_all("td")
        # La page a 4 colonnes utiles: [vide/padding], [modèle], [version], [EOL?]
        if len(cells) < 4:
            continue

        model_cell = clean(cells[1])   # 2e colonne : modèle (ex: "FortiGate-40F")
        eol_cell   = clean(cells[3])   # 4e colonne : "Y" ou "N"

        # Cas des VM : la page peut regrouper sous "Virtual Machines"
        if normalized_model.startswith("VM"):
            found = ("Virtual Machines" in model_cell) or bool(pat.search(model_cell))
        else:
            found = bool(pat.search(model_cell))

        if found:
            # On tolère d'éventuels espaces/commentaires: on teste juste la première lettre
            is_eol = eol_cell.upper().startswith("Y")
            if is_eol:
                result = (f"Modèle {modele} en fin de vie (end of life) - "
                          f"Plus supporté par Fortinet.\n Source : {url}")
                conform = False
            else:
                result = (f"Modèle {modele} avec un support toujours actif par Fortinet.\n"
                          f"Source : {url}")
                conform = True
    ...
    result = f"Modèle {modele} : Non trouvé dans la liste officielle"
    conform = None
    print(f"Result: {result}")
    print(f"Conform: {conform}")
    return result, conform
...
def verifier_logs_par_regle(config_lines):
    ...
    # Dictionnaire pour stocker le statut de logtraffic de chaque règle
    # Exemple : rule_logs = { '1': 'all', '2': 'disable', '3': 'utm', ... }
    rule_logs = {}

    # Par défaut, on considère qu'il n'y a pas d'implicit deny en log "enable"
    # On met 'disable' par défaut. Si on trouve "set fwpolicy-implicit-log enable",
    # on passera 'implicit_deny_logging' à 'all'
    implicit_deny_logging = 'disable'

    # Première passe : on cherche si "set fwpolicy-implicit-log enable" est présent
    for line in config_lines:
        if 'set fwpolicy-implicit-log enable' in line.strip().lower():
            implicit_deny_logging = 'all'
            break  # Pas besoin d'aller plus loin, on a trouvé qu'il est activé

    for line in config_lines:
        line = line.strip()

        # Début de la section firewall policy
        if 'config firewall policy' in line:
            in_firewall_policy = True
            continue

        # Fin de la section firewall policy
        elif line == 'end' and in_firewall_policy:
            in_firewall_policy = False
            continue
        ...
        if in_firewall_policy:
            ...
            if line.startswith('edit '):
                # On récupère l'ID de la règle
                current_rule_id = line.split(' ')[1].strip('"')
                # On initialise le statut à None,
                # signifiant qu'on n'a pas encore rencontré de set logtraffic
                rule_logs[current_rule_id] = None

            # Détection de la configuration de logtraffic
            elif 'set logtraffic' in line and current_rule_id:
                if 'all' in line:
                    rule_logs[current_rule_id] = 'all'
                elif 'disable' in line:
                    rule_logs[current_rule_id] = 'disable'

            # Fin de la règle (FortiGate : 'next')
            elif line == 'next':
                # Si, à ce stade, rule_logs[current_rule_id] est encore None,
                # alors on n'a pas rencontré `set logtraffic`, donc on le met en 'utm'
                # (ou tout autre comportement que vous souhaitez considérer par défaut)
                if rule_logs[current_rule_id] is None:
                    rule_logs[current_rule_id] = 'utm'
                # On remet l'ID de la règle à None (fin de traitement de cette règle)
                current_rule_id = None

    # Maintenant, on organise les résultats sous forme de listes
    rules_with_logs_all = [rule_id for rule_id, status in rule_logs.items() if status == 'all']
    rules_with_logs_disabled = [rule_id for rule_id, status in rule_logs.items() if status == 'disable']
    rules_with_logs_utm = [rule_id for rule_id, status in rule_logs.items() if status == 'utm']

    # On peut également compter le nombre de règles dans chaque catégorie
    ...
    # - Si fwpolicy-implicit-log == enable => on incrémente "all"
    # - Sinon => on incrémente "disable"
    if implicit_deny_logging == 'all':
        nb_all += 1
    else:
        nb_disable += 1

    # Vous pouvez retourner ces informations de la manière qui vous arrange.
    return {
...
def verifier_usage_by_sequence(config_lines):
    ...
    for line in config_lines:
        ...
        # Analyse uniquement à l'intérieur de la section
        if in_firewall_policy:
            # Nouvelle règle
            if line.startswith('edit '):
                current_rule_id = line.split(' ')[1].strip('"')

            # Vérifie la présence d'un global-label
            elif line.startswith('set global-label '):
                by_sequence_detected = True

            # Vérifie le multi-interface dans srcintf ou dstintf
            elif line.startswith('set srcintf ') or line.startswith('set dstintf '):
                parts = line.split('"')
                interfaces = [p for i, p in enumerate(parts) if i % 2 != 0]
                if len(interfaces) > 1:
                    by_sequence_detected = True
    ...
    if by_sequence_detected:
        return "Détection de policy avec plusieurs interfaces ou global-label. Cela indique que le client fait du 'By Sequence'", True
    else:
        return "Aucune policy avec plusieurs interfaces ni 'set global-label' détectée.", False
...
ACCEPTED_AUTHENTICATIONS = {'sha256', 'sha384', 'sha512'}
...
def verifier_algorithmes_chiffrement_auth(config_lines):
    ...
    return "Algorithmes de chiffrement et d'authentification conformes sur toutes les phases", True, ""
...
def verifier_mfa_utilisateurs_admins(config_lines):
    ...
    for line in config_lines:
        ...
        if line == 'config user local':
            ...
            if line.startswith('edit '):
                current_user = {'Nom du compte': '', 'MFA': 'No'}
                if '"' in line:
                    current_user['Nom du compte'] = line.split('"')[1]
            elif current_user and 'set two-factor' in line:
                current_user['MFA'] = line.split(' ')[2]
            elif line == 'next' and current_user:
                if current_user['MFA'] == 'No':
    ...
    for line in config_lines:
        ...
        if line == 'config system admin':
            ...
            if line == 'config' or line.startswith('config '):
                ...
                current_admin = {'Nom du compte': '', 'MFA': 'No'}
                current_admin['Nom du compte'] = line.split('"')[1]
            elif current_admin and 'set two-factor' in line:
                current_admin['MFA'] = line.split(' ')[2]
            elif line == 'next' and current_admin and depth == 0:
                if current_admin['MFA'] == 'No':
                    mfa_absent_admins.append(current_admin['Nom du compte'])
                current_admin = None

    if mfa_absent_users or mfa_absent_admins:
        user_message = f"Comptes utilisateurs sans MFA : {', '.join(mfa_absent_users)}" if mfa_absent_users else ""
        admin_message = f"Admins sans MFA : {', '.join(mfa_absent_admins)}" if mfa_absent_admins else ""
        mfa_absent_accounts = mfa_absent_users + mfa_absent_admins
        if len(mfa_absent_accounts) > 3:
            return "Plusieurs comptes sont configurés sans MFA. Voir la liste complète 'Accounts' pour plus de détails.", False
        elif len(mfa_absent_accounts) == 1:
            return f"MFA absente pour le compte suivant : {mfa_absent_accounts[0]}", False
        else:
            return f"{user_message}\n{admin_message}", False
    return "MFA présente pour tous les comptes utilisateurs et administrateurs", True
...
def read_active_interfaces(file_path):
    ...
    wan_patterns = [r"\bwan\b", r"\bfibre\b", r"\bftto\b", r"\bftth\b", r"\binternet\b", r"\badsl\b", r"\b4g\b"]
    ...
    return {iface: d for iface, d in interfaces.items() if d['active']}
...
def verifier_ldaps(config_lines):
    ...
    ldap_found = False  # Nouvelle variable pour indiquer qu'au moins une connexion LDAP a été détectée
    ...
    for line in config_lines:
        ...
        if line.startswith('config user ldap'):
            ...
            ldap_found = True  # On a trouvé une connexion LDAP
            if '"' in line:
        ...
        elif in_ldap_block and 'set secure ldaps' in line:
            secure_ldaps_found = True

        elif in_ldap_block and line == 'next':
            if current_ldap and not secure_ldaps_found:
                ldap_connections.append(current_ldap)
            current_ldap = None

        elif in_ldap_block and line == 'end':
            in_ldap_block = False

    # Maintenant, en fin de traitement :
    if not ldap_found:
        # Aucune connexion LDAP trouvée du tout
        return "Aucune synchronisation LDAP(S) identifiée", "N/A"
    elif ldap_connections:
        # Des connexions LDAP non sécurisées existent
        return f"Présence LDAP mais pas LDAPS pour les connexions suivantes : '{', '.join(ldap_connections)}'", "Non"
    else:
        # Au moins une connexion LDAP sécurisée trouvée et aucune non sécurisée
        return "Présence LDAPS pour toutes les connexions LDAP", "Oui"
...
def verifier_utilisation_sdwan(config_lines, selected_wan_interfaces):
    ...
    for line in config_lines:
        ...
        # Gestion des sous-blocs dans SD-WAN
        if in_sdwan_block:
            if line == 'config zone':
                in_zone_block = True
                ...
                if 'set interface' in line:
                    ...
                    if len(parts) > 1:
                        ...
                        sdwan_members.add(interface_name)
    ...
    # Déterminer si le SD-WAN est configuré : si on a trouvé au moins un membre
    sdwan_configured = len(sdwan_members) > 0

    if not sdwan_configured:
        return "Le SD-WAN n'est pas configuré", False, "Configurer le SD-WAN", missing_interfaces

    for iface in selected_wan_interfaces:
        if iface in physical_interfaces and iface not in sdwan_members:
...
def verifier_profils_securite_sur_regles(config_lines):
    ...
    Scanne l'intégralité de la section 'config firewall policy' de manière robuste.
    """
    ...
    # Première passe : vérifier la présence d'une section firewall policy
    has_firewall_policy = any('config firewall policy' in line for line in config_lines)
    if not has_firewall_policy:
        return "Aucune section 'config firewall policy' trouvée dans la configuration.", True, []
    ...
    for line in config_lines:
        ...
        # Analyse interne uniquement si on est dans la bonne section
        if in_firewall_policy:

            # Nouvelle règle détectée
            if line.startswith('edit '):
                current_rule_id = line.split(' ')[1].strip('"')
                logs_are_utm = True  # par défaut, on considère UTM tant que pas 'all' ou 'disable'
                security_profile_enabled = False

            elif line.startswith('set logtraffic '):
                # Gestion des modes de log
                if 'disable' in line:
                    logs_are_utm = False
                elif 'all' in line:
                    logs_are_utm = False
            ...
            elif 'set utm-status enable' in line:
                security_profile_enabled = True

            elif line == 'next':
                # Fin de règle → vérification
    ...
    if rules_with_utm_logs_no_security_profile:
        return (
            f"Règles avec logs UTM sans profils de sécurité activés : "
...
def verifier_webfilter_profiles(config_lines, licence_utm):
    ...
    in_profile_group = False  # On est dans un bloc de groupe de profils
    in_profile_group_edit = False  # On édite un groupe de profils
    current_edit_group = None  # Groupe de profils en cours d'édition
    ...
    if not licence_utm:
        ...
        if non_conforming_web_filter:
            ...
            for profile, issues in non_conforming_web_filter.items():
                ...
                details.append(f"{profile} ({issues_str})")
            if len(non_conforming_web_filter) == 1:
                result_web_filter.append("Le profil de WebFilter suivant n'est pas conforme : " + " , ".join(details))
            else:
                result_web_filter.append(
...
def collect_schedule_data(config_lines: List[str], current_date: datetime) -> Tuple[int, int, int]:
    ...
    """
    # Variables pour stocker les schedules
    onetime_schedules = {}  # {nom_schedule: date_fin}
    recurring_schedules = set()  # Ensemble des noms de schedules récurrents
    policy_schedules = []  # Liste des schedules utilisés dans les policies

    # Flags pour identifier le bloc courant
    in_onetime = False
    in_recurring = False
    in_policy = False
    current_edit = None
    ...
    for line in config_lines:
        line = line.strip()
        # Détection des blocs de configuration
        ...
        if in_onetime:
            if line.startswith('edit '):
                ...
                try:
                    ...
                    print(f"Erreur de format de date pour le schedule '{current_edit}': '{end_datetime_str}'")
            elif line.startswith('next'):
                current_edit = None
            continue

        # Traitement des blocs recurring
        if in_recurring:
            if line.startswith('edit '):
                current_edit = line.split(' ')[1].strip('"')
                # Vérifier si ce n'est pas "always"
        ...
        # Traitement des blocs policy
        if in_policy:
            if line.startswith('set schedule'):
                schedule_name = line.split(' ')[2].strip('"')
                policy_schedules.append(schedule_name)
            elif line.startswith('next'):
                continue
            continue
    ...
    # Catégorisation des règles
    for schedule in policy_schedules:
        if schedule.lower() == "always":
            always_count += 1
        elif schedule in recurring_schedules:
            scheduled_actif_count += 1
        elif schedule in onetime_schedules:
            end_date = onetime_schedules[schedule]
            if end_date < current_date:
                scheduled_expire_count += 1
            else:
                scheduled_actif_count += 1
        else:
            # Cas où le schedule n'est ni "always", ni récurrent, ni onetime connu
            print(f"Avertissement : Schedule inconnu '{schedule}'")
...
def auditer():
    ...
    progress_bar.place(relx=0.5, rely=0.94, anchor=tk.CENTER)
    ...
    if imported_filepath:
        ...
        filename = creer_rapport_excel(
            guest_result, guest_conform,
            admin_result, admin_conform,
            usb_result, usb_conform,
            vs_any_result, vs_any_conform, vip_any_result, vip_any_conform,
            utilisation_ssl_result, utilisation_ssl_conform,
            all_in_rules_result, all_in_rules_conform, all_in_rules_ids,
            geo_ip_result, geo_ip_conform,
            hostname, version_fortigate_result,
            version_conform, isdb_result, isdb_conform,
            version_cve_message, http_https_result,
            eol_result, eol_conform,
            http_https_conform, http_https_enabled_interfaces, interfaces_access_details, admin_sns_result,
            admin_sns_conform, pki_sns_result,
            pki_sns_conform, pki_pkisns_result,
            pki_pkisns_conform, deny_implicit_result,
            deny_implicit_conform, sync_fortianalyzer_result,
            ...
            ha_session_pickup_result, ha_session_pickup_conform,
            ha_redundance_result, ha_redundance_conform,
            ha_override_result, ha_override_conform,
        ...
        filename_word = creer_rapport_word(hostname, version, model, config_lines, version_conform, eol_conform, sauvegardes_conform, usb_conform, result_port_https_admin, is_compliant_https,
                                           ...
                                           ha_session_pickup_result, ha_session_pickup_conform, ha_redundance_result, ha_redundance_conform, ha_override_result, ha_override_conform,
        ...
        messagebox.showinfo("Audit terminé",
                    f"L'audit est terminé.\n"
                    f"Rapport Excel : {filename}\n"
                    f"Rapport Word : {filename_word}")
    else:
        # Masquer la barre même en cas d'erreur
        progress_bar.place_forget()
        messagebox.showerror("Erreur", "Aucun fichier importé !")
...
def creer_rapport_excel(guest_result, guest_conform, admin_result, admin_conform, usb_result, usb_conform, vs_any_result, vs_any_conform, vip_any_result, vip_any_conform, utilisation_ssl_result, utilisation_ssl_conform, all_in_rules_result, all_in_rules_conform, all_in_rules_ids, geo_ip_result, geo_ip_conform, hostname,
                            ...
                            ha_session_pickup_result, ha_session_pickup_conform, ha_redundance_result,
                            ha_redundance_conform, ha_override_result,
        ...
        audit_data = [
...

            {
                ...
                "accord": 'OUI',
                "action_sns": '',
                "action": '' if version_conform else "Faire un trade-up de Fortigate"
            },
...

            {
                ...
                "benefice": "L’auto-installation d'une configuration ou d'un firmware via USB peut permettre à un attaquant local de compromettre le FortiGate",
                "result": usb_result,
            ...
            {"point": "Contrôle absence de VIP avec any en interface",
             ...
             "action": 'Ne pas mettre "any" en interface externe des VIP' if not vip_any_conform else ''},
            ...
            {"point": "Contrôle absence de Virtual Server avec any en interface",
             ...
             "action": 'Ne pas mettre "any" en interface externe des Virtual Servers' if not vs_any_conform else ''},
            ...
            {"point": "Utilisation LDAPS",
             ...
             "action": "Mettre en place du LDAPS" if ldaps_conform == "Non" else ''},
            ...
            {"point": "Activer le session pickup",
             ...
             "accord": 'OUI',
             "action": "Activer le session pickup et les options associées" if ha_session_pickup_conform == False else ''},
        ...
        img = OpenpyxlImage(r"Références/sns-security-logo1.png")
        img.anchor = 'G2'
        ws.add_image(img)
        ...
        format_merged_row(ws, 'A45', 'G45')
        format_merged_row(ws, 'A50', 'G50')
        format_merged_row(ws, 'A62', 'G62')

        categories_map = {
            "Catégorie : Système": range(8, 13),
            "Catégorie : Administration & comptes": range(14, 25),
            "Catégorie : Réseau et flux": range(26, 40),
            "Catégorie : HA - Cluster": range(41, 44),
            "Catégorie : VPN": range(45, 49),
            "Catégorie : Profils de sécurité": range(50, 61),
            "Catégorie : WIFI": range(62, ws.max_row + 1)
        }
        ...
        img_sans_accord = OpenpyxlImage(r"Références/sns-security-logo-actions.png")
        ...
        ws_sans_accord.column_dimensions['A'].width = 45
        ...
        img_avec_accord = OpenpyxlImage(r"Références/sns-security-logo-actions.png")
        ...
        ws_avec_accord.column_dimensions['A'].width = 45
        ...
        for row in ws_stats.iter_rows(min_row=1, max_row=85, min_col=1, max_col=4):
            for cell in row:
        ...
        img_stats = OpenpyxlImage(r"Références/sns-security-logo1.png")
        img_stats.anchor = 'D2'
        ...
        cell = ws_stats['A6']
        cell.value = (
            "Notre audit a permis d'analyser plusieurs caractéristiques des règles de votre pare-feu : statut, type de log, type de schedule."
        ...
        # --- Bordure de droite : colonne D (lignes 1..79) ---
        for row in range(1, 86):
            cell = ws_stats.cell(row=row, column=4)
            original_border = cell.border
            new_border = Border(
                left=original_border.left,
                right=thin_side,  # on ajoute/renforce la droite
                top=original_border.top,
                bottom=original_border.bottom
            )
            cell.border = new_border

        # --- Bordure du bas : ligne 59 (colonnes A..D) ---
        for col in range(1, 5):
            cell = ws_stats.cell(row=59, column=col)
            original_border = cell.border
            new_border = Border(
                left=original_border.left,
                right=original_border.right,
        ...
        # --- Bordure de gauche : colonne A (lignes 7..79) ---
        ...
        # Graphique pour comparer le nombre de règles activées/désactivées
        ws_stats["B64"] = "État des règles"
        ws_stats["C64"] = "Nombre"
        ws_stats["B65"] = "Règles activées"
        ws_stats["C65"] = nb_policy_enable
        ws_stats["B66"] = "Règles désactivées"
        ws_stats["C66"] = nb_policy_disabled

        # Création du PieChart
        pie_statut = PieChart()
        labels3 = Reference(ws_stats, min_col=2, min_row=65,
                            max_row=66)  # Colonne A : "Règles activées"/"Règles désactivées"
        data3 = Reference(ws_stats, min_col=3, min_row=64, max_row=66)  # Colonne B : contient le titre + les 2 valeurs
        ...
        pie_statut.title = "Règles activées vs désactivées"
        ...
        ws_stats.add_chart(pie_statut, "B64")

        total_rules = nb_policy_enable + nb_policy_disabled
        if total_rules > 0:
            percent_disabled = round(nb_policy_disabled * 100 / total_rules, 1)
        else:
            percent_disabled = 0

        ws_stats["B62"] = "Ce graphique montre la répartition des règles de votre pare-feu en fonction de leur statut :"

        if nb_policy_disabled > 0:
            # On calcule le pourcentage arrondi pour l’afficher
            percent_disabled = round(nb_policy_disabled * 100 / total_rules, 1)
            resultat_pie3 = (
                f"Vous avez {nb_policy_enable} règles activées contre {nb_policy_disabled} règle(s) désactivée(s). Est-ce que les règles désactivées sont légitimes ?"
            )
        else:
            resultat_pie3 = (
        ...
        ws_stats["A81"] = resultat_pie3

        # Alignement (wrap_text pour gérer les sauts de ligne)
        ws_stats["A81"].alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
        ws_stats["A81"].font = Font(size=12)


        # Graphique pour la répartition des règles par type de logs
        ...
        if nb_policy_always == total_rules_schedule and total_rules_schedule > 0:
            message_schedule = (
                ...
                "Est-ce vraiment nécessaire ou est-ce que vous pouvez restreindre ?"
            )
        ...
        img_accounts = OpenpyxlImage("Références/sns-security-logo-account.png")
        ...
        ws_accounts.column_dimensions["A"].width = 45
...
def creer_rapport_word(hostname, version, model, config_lines, version_conform, eol_conform, sauvegardes_conform, usb_conform, result_port_https_admin, is_compliant_https,
                       ...
                       objets_result, objets_conform, objets_non_utilises, result_profils, conformity_profils, rule_ids_profils, blackhole_result, blackhole_conform, ports_deny_result, ports_deny_conform, fortiguard_result, fortiguard_conform, fortisandbox_result, fortisandbox_conform,
                       sequence_result, sequence_conform):
    modele = "Références/generique.docx"
    current_time = datetime.now().strftime("%Y-%m-%d-%Hh%Mm%Ss")
    ...
    doc.add_paragraph('')
    doc.add_paragraph().add_run("Point audité :").bold = True
    doc.add_paragraph("Vérification que votre modèle FortiGate bénéficie toujours d’un support actif et des mises à jour fournies par Fortinet.")
    doc.add_paragraph('')
    txt = doc.add_paragraph()
    run_result = txt.add_run("Résultat : ")
    run_result.bold = True
    ...
    doc.add_paragraph('')
    doc.add_paragraph().add_run("Point audité :").bold = True
    doc.add_paragraph("Vérification que les options 'set revision-backup-on-logout' et 'set revision-image-auto-backup' sont activées. Elles permettent la création automatique de backup de FortiGate lors d'un upgrade ou à chaque logout d'un compte admin.")
    doc.add_paragraph('')
    txt = doc.add_paragraph()
    run_result = txt.add_run("Résultat : ")
    run_result.bold = True
    ...
    doc.add_paragraph('')
    doc.add_picture('Références/ISDB.png', width=Inches(5.5))
    doc.add_paragraph('')
    txt = doc.add_paragraph()
    run_result = txt.add_run("Résultat : ")
    run_result.bold = True
    ...
    doc.add_paragraph().add_run("Point audité :").bold = True
    doc.add_paragraph(
        "Les Virtuals Servers sont utilisées lorsqu’on veut faire rentrer du flux de l’extérieur vers l’interne à destination d'un serveur par exemple. "
    ...
    if not vs_any_conform:
        risque = add_risk_table(
            ...
            impact="SIGNIFICATIF",
            correction="SIMPLE",
            remediation="Ne pas utiliser de ANY sur les Virtual Server"
        )
        risques.append(risque)
        risk_number += 1
        doc.add_paragraph('')
    ...
    doc.add_paragraph('')
    doc.add_picture('Références/CTI.png', width=Inches(6.7))
    txt = doc.add_paragraph()
    run_result = txt.add_run("Résultat : ")
    run_result.bold = True
    ...
    if conformity_profils:
        ...
        run_status.font.color.rgb = RGBColor(0xCC, 0x00, 0x00)  # Rouge #CC0000
    doc.add_paragraph(
        f"Absence de règles avec logs en UTM sans profil de sécurité actif." if conformity_profils else
        f"{result_profils}.")
    doc.add_paragraph('')
    doc.add_paragraph('')
    ...
    doc.add_paragraph().add_run("Point audité :").bold = True
    doc.add_paragraph(
        "Utilisation du LDAPS pour les connecteurs AD. Le LDAPS permet un chiffrement des données, offrant une protection accrue contre les interceptions malveillantes.")
    doc.add_paragraph('')
    txt = doc.add_paragraph()
    run_result = txt.add_run("Résultat : ")
    run_result.bold = True
    run_status = txt.add_run("CONFORME" if ldaps_conform else "NON CONFORME")
    run_status.bold = True
    if not ldaps_conform:
        run_status.font.color.rgb = RGBColor(0x00, 0xB0, 0x50)  # Vert #00B050
    else:
        run_status.font.color.rgb = RGBColor(0xCC, 0x00, 0x00)  # Rouge #CC0000
    doc.add_paragraph(
        f"{ldaps_result}. " if  ldaps_conform else
        f"{ldaps_result}. "
    )
    doc.add_paragraph('')
    doc.add_paragraph('')
    ...
    if ha_details["ha_present"]:
        ...
        doc.add_paragraph().add_run("Point audité :").bold = True
        doc.add_paragraph(
            "Vérification si l'option 'session pickup' est activée. Cette option permet de minimiser les interruptions de communication, évitant de devoir redémarrer les sessions actives.")
        doc.add_paragraph('')
        txt = doc.add_paragraph()
        run_result = txt.add_run("Résultat : ")
        run_result.bold = True
        ...
        doc.add_paragraph().add_run("Point audité :").bold = True
        doc.add_paragraph(
            "Vérification si les interfaces de HA sont redondées avec au minimum 2 heartbeats. Cela permet d'assurer la redondance pour la synchronisation du cluster.")
        doc.add_paragraph('')
        txt = doc.add_paragraph()
        run_result = txt.add_run("Résultat : ")
        run_result.bold = True
        ...
        doc.add_paragraph('')
        doc.add_picture('Références/cluster.png', width=Inches(6.5))
        doc.add_paragraph('')
        doc.add_paragraph('')
    ...
    if not ike_result == "Absence de tunnel IPSEC configuré":
        ...
        doc.add_paragraph(
            ...
            " et ne doivent plus être utilisés. L’ANSSI recommande AES256 pour le chiffrement et SHA256 "
            "(ou supérieur de la famille SHA2) pour l’intégrité, considérés comme l’équilibre actuel entre robustesse et performance.")
        doc.add_paragraph('')
        txt = doc.add_paragraph()
        run_result = txt.add_run("Résultat : ")
        run_result.bold = True
    ...
    doc.add_picture('Références/Security_profiles.png', width=Inches(6.7))
    doc.add_paragraph('')
    doc.add_paragraph(
    ...
    if licence_utm:
        ...
        if not dnsfilter_conformity:
            risque = add_risk_table(
                ...
                impact="GRAVE",
                correction="RAISONNABLE",
                remediation="Utiliser le DNS Filter"
            )
            risques.append(risque)
            risk_number += 1
            doc.add_paragraph('')
        ...
        if not conformity_web_filter:
            risque = add_risk_table(
                ...
                impact="GRAVE",
                correction="RAISONNABLE",
                remediation="Utiliser le web-filter"
            )
            risques.append(risque)
            risk_number += 1
            doc.add_paragraph('')
        ...
        doc.add_paragraph('')
        txt = doc.add_paragraph()
        doc.add_paragraph().add_run("Résultat :").bold = True
        ...
        if not av_conformity:
            risque = add_risk_table(
                ...
                impact="GRAVE",
                correction="RAISONNABLE",
                remediation="Utiliser l'antivirus de FortiGuard"
            )
            risques.append(risque)
            risk_number += 1
            doc.add_paragraph('')
...
def exporter_utilisateurs_admins():
    config_lines = lire_config_fortigate(imported_filepath)
...
token = None  # Variable globale partagée entre les threads
...
def login_azure():
    CLIENT_ID = "7ab3302a-045d-4126-af62-3d7e1cefc56e"
    TENANT_ID = "60077944-a6d7-4e79-80cf-2c8320e09186"
    AUTHORITY = f"https://login.microsoftonline.com/60077944-a6d7-4e79-80cf-2c8320e09186"
    SCOPE = ["User.Read"]
    ...
    if "access_token" in result:
        print("✅ Connecté avec succès (nouveau login)")
        save_cache(cache)

        # Vérification de sécurité : utilisateur du tenant SNS uniquement
        claims = result.get("id_token_claims", {})
        tid = claims.get("tid")
        upn = claims.get("preferred_username", "")
        if tid != TENANT_ID or not upn.endswith("@sns-security.fr"):
            print("⛔ Utilisateur non autorisé :", upn)
            return None

        return result
    else:
        print("❌ Erreur de connexion :", result.get("error"))
        return None
...
def auth_thread():
    ...
    if token is None:
        root.after(0, lambda: (
            messagebox.showerror("Erreur", "Échec d'authentification."),
            root.destroy()
        ))

def wait_for_login():
    """Attente non bloquante de la connexion."""
    global token
    if token:
        root.deiconify()  # affiche la fenêtre principale
    else:
        root.after(500, wait_for_login)
...
root = tk.Tk()
root.iconbitmap("Références/shield.ico")
root.withdraw()  # Cache la fenêtre tant que l’utilisateur n’est pas authentifié

# Lancement de l’authentification dans un thread parallèle
threading.Thread(target=auth_thread, daemon=True).start()
wait_for_login()

root.title("Audit de configuration FortiGate")
# Chargement de l'image
img = Image.open(r"Références/image_fond.jpg")
photo = ImageTk.PhotoImage(img)
# Création d'un Frame pour organiser les éléments
frame = tk.Frame(root)
frame.pack(expand=True)
# Ajout de l'image en tant que fond du Frame
label_image = tk.Label(frame, image=photo)
label_image.pack(expand=True)

# Création de variables IntVar pour les cases à cocher
licence_utm_var = IntVar()
lien_mpls_L2L_var = IntVar()

# Ajout de la case à cocher "Licence UTM"
licence_utm_checkbox = tk.Checkbutton(frame, text="Licence UTM\n valide", variable=licence_utm_var, bg="white")
licence_utm_checkbox.place(relx=0.75, rely=0.042)
licence_utm_checkbox.config(font=('Helvetica', 13, 'bold'))

# Ajout de la case à cocher "Présence lien MPLS ou L2L"
lien_mpls_L2L_checkbox = tk.Checkbutton(frame, text="Présence lien\n MPLS ou L2L", variable=lien_mpls_L2L_var, bg="white")
lien_mpls_L2L_checkbox.place(relx=0.08, rely=0.042)
lien_mpls_L2L_checkbox.config(font=('Helvetica', 13, 'bold'))

# Ajout du bouton "Importer" sur l'image
importer_button = tk.Button(frame, text="Importer le fichier\nde configuration", command=importer_fichier,
                            bg="RoyalBlue", fg="white")
importer_button.place(relx=0.38, rely=0.035)
importer_button.config(width=15, height=2, font=('Helvetica', 16, 'bold'))

# Ajout d'un label pour afficher le nom du fichier importé
fichier_label = tk.Label(frame, text="", bg="white", anchor='w', justify='left')
fichier_label.place(relx=0.38, rely=0.15, anchor=tk.W)

# Ajout du bouton "Charger les interfaces et zones"
load_interfaces_button = tk.Button(frame, text="Charger les interfaces et zones", command=load_wan_interfaces, bg="RoyalBlue",
                                   fg="white")
load_interfaces_button.place(relx=0.52, rely=0.25, anchor=tk.CENTER)
load_interfaces_button.config(width=25, height=2, font=('Helvetica', 16, 'bold'))

# Encadré "WAN interfaces and Zones WAN à inclure"
wan_list_label = tk.Label(frame, text="Interfaces WAN et zones WAN à sélectionner :", font=("Arial", 14, "bold"))
wan_list_label.place(relx=0.5, rely=0.32, anchor=tk.CENTER)

wan_list_frame = tk.Frame(frame, bg="white", width=450)
wan_list_frame.place(relx=0.5, rely=0.57, anchor=tk.CENTER, width=500)

wan_scrollbar = Scrollbar(wan_list_frame, orient="vertical")
wan_canvas = tk.Canvas(wan_list_frame, yscrollcommand=wan_scrollbar.set, height=350, bg="white", width=500)
wan_scrollbar.config(command=wan_canvas.yview)
wan_scrollbar.pack(side="right", fill="y")
wan_canvas.pack(side="left", fill="both", expand=True)

wan_frame = tk.Frame(wan_canvas, bg="white", width=500)
wan_canvas.create_window((0, 0), window=wan_frame, anchor="nw")

# Ajout du bouton "Auditer"
auditer_button = tk.Button(frame, text="Auditer", command=auditer, bg="RoyalBlue", fg="white")
auditer_button.place(relx=0.5, rely=0.87, anchor=tk.CENTER)
auditer_button.config(width=15, height=2, font=('Helvetica', 18, 'bold'))
