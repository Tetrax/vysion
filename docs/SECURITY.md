...
- aucune IP Docker statique ni aucun réseau d'entreprise imposé (`Subnet-Docker` supprimé) : le seul rattachement possible est celui du standalone, **optionnel et vide par défaut** (`DOCKER_NETWORK`, `DOCKER_NETWORK_EXTERNAL`, `IPV4_ADDRESS`) — aucun subnet, aucune adresse ni aucun port d'entreprise n'est versionné ;
