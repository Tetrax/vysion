...
"""
    ...
    assert [finding.status for finding in _findings(raw)] == [
        AuditStatus.UNKNOWN,
        AuditStatus.UNKNOWN,
        AuditStatus.UNKNOWN,
    ]
...
"""
    ...
    assert _findings(raw)[1].status is AuditStatus.UNKNOWN
...
def test_explicit_wan_without_ssh_is_pass() -> None:
    raw = """config system interface
    edit "wan1"
...
"""
    ...
    assert _findings(raw)[1].status is AuditStatus.PASS
...
"""
    ...
    assert _findings(raw)[1].status is AuditStatus.FAIL
...
"""
    ...
    assert _findings(raw)[2].status is AuditStatus.UNKNOWN
...
"""
    ...
    assert _findings(raw)[2].status is AuditStatus.PASS
...
"""
    ...
    assert _findings(raw)[2].status is AuditStatus.FAIL
...
def test_unknown_sections_keys_and_subsections_are_ignored() -> None:
    raw = """config system interface
    edit "wan1"
        set vdom root
        set role wan
